...
VALUES (1, 1, NULL, '用户可以使用改名卡，更改自己的名字。blackchat名称全局唯一，快抢订你的专属昵称吧',
...
INSERT INTO `item_config`
VALUES (6, 2, 'https://minio.blackchat.cn/blackchat/%E8%B4%A1%E7%8C%AE%E8%80%85.png', '抹茶项目contributor专属徽章',
        '2023-05-07 17:50:31.109', '2023-05-07 17:56:36.059');
...
(
    ...
    `status`        int(11)                                                                DEFAULT 0 COMMENT '使用状态 0.正常 1.拉黑',
    `active_status` int(11)                                                                DEFAULT 1 COMMENT '在线状态 1.在线 2.离线',
    `create_time`   datetime(3)                                                   NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    `update_time`   datetime(3)                                                   NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '修改时间',
    PRIMARY KEY (`id`) USING BTREE,
  ...
  ROW_FORMAT = Dynamic;
INSERT INTO `blackchat`.`user` (`id`, `name`, `avatar`, `sex`, `open_id`, `last_opt_time`, `ip_info`, `item_id`,
...
ALTER TABLE `user`
    add COLUMN `active_status` INT(11) DEFAULT 2 COMMENT '在线状态 1在线 2离线' AFTER `open_id`;

ALTER TABLE `user`
    add KEY `idx_active_status_last_opt_time` (`active_status`, `last_opt_time`);
...
values (1, 1, '抹茶全员群', 'https://blackchat.cn/assets/logo-e81cd252.jpeg');
